...
def monitoring():
    ...
    # Only read filters if user is logged in
    ...
    # Only apply filters if logged in
    if hasattr(current_user, "is_authenticated") and current_user.is_authenticated:
        ...
        if search_branch:
            ...
            params.extend([f'%{search_branch}%', f'%{search_branch}%'])
        if search_serial:
            query += " AND ws.serial_number LIKE ?"
            params.append(f'%{search_serial}%')
        if search_brand:
            query += " AND ws.brand LIKE ?"
    ...
    try:
        ...
        brands = db.execute("SELECT DISTINCT brand FROM weighing_scales ORDER BY brand").fetchall()
        ...
        return "An error occurred while loading monitoring data", 500
...
def edit_scale(id):
    ...
    if request.method == 'POST':
        ...
        # Convert old_data to dict and handle datetime/date objects
        old_data_dict = None
        if old_data:
            old_data_dict = {}
...
def scale_detail(id):
    ...
    if scale['license_expiry_date']:
        ...
        if days_diff < 0:
            ...
            scale['expiry_message'] = f"Active ({days_diff} days left)"
